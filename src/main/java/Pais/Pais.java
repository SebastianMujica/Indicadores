...
@Entity()
@Table(name="tblsis_pais")
@Cacheable()
@Cache(usage = CacheConcurrencyStrategy.READ_ONLY)
@Inheritance(strategy = InheritanceType.SINGLE_TABLE)
//@org.hibernate.annotations.BatchSize(size = 10)

public  class Pais extends DtoBase {
  ...
  @NotNull(message="{campo_requerido}") @Size(min=3,max=3,message="{longitud}") String strsigla;
  
  @JoinColumn(name="pais_lngid") 
  @OneToMany(cascade=CascadeType.ALL,fetch=FetchType.LAZY)
  //@BatchSize(size=25)
  @LazyCollection(LazyCollectionOption.FALSE) List<Estado> estados;
  ...
  public void setEstados(  List<Estado> param){
    this.estados=param;
  }
