...
import Municipio.vistaMunicipio;
import Parroquia.modeloParroquia;
public class controladorMunicipio extends ControladorBase implements ActionListener, MouseListener, KeyListener {
  public controladorMunicipio(){
    vistaMunicipio vista=new vistaMunicipio(this);
  ...
  public void actualizarVista(){
    vistaMunicipio vista=(vistaMunicipio)this.getSession().getVista();
    Municipio dto=new Municipio();
    modeloMunicipio modMunicipio=new modeloMunicipio();
    this.getSession().setListaDto(modMunicipio.buscar(dto));
    modeloReporte modeloReporte=new modeloReporte();
    javax.swing.DefaultComboBoxModel modeloComboReporte=new javax.swing.DefaultComboBoxModel(modeloReporte.buscar(new Reporte(),"strpadre","Municipio").toArray());
    ...
    vista.getCmbdtoestado().setModel(modeloComboEstado);
    modeloParroquia modeloParroquia=new modeloParroquia();
    //vista.getLstparroquias().setModelDes(modeloParroquia.buscarParroquias());
    vista.getLstparroquias().setModelDes(new Object[0]);
    this.getSession().setDtoActual(0,dto);
    this.cargarGrid();
  }
  public void nuevo(){
	    vistaMunicipio vista=new vistaMunicipio(this);
	    vista=(vistaMunicipio)this.getSession().getVista();
	    vista.setDto(new Municipio());
	  ...
	  public void buscar(){
	    ...
	    if (str.isEmpty() || str.length()==0 || str.contentEquals("0")){
	       vista.mensageDialogo("error","Valor de búsqueda vacío o inválido.\nIntroduzca un número mayor que cero." , vista.getTitle());
	    	...
	    	modeloMunicipio modelo=new modeloMunicipio();
	    	Municipio dto=new Municipio();
	        dto=modelo.buscarMunicipio(new Municipio(),"lngid",str);
	        vista.setDto(dto);		
	    }
	  }
	  ...
	  public void siguiente(){
		  if (this.getSession().getListaDto().size()>0){
		    	this.getSession().setDtoActual(this.getSession().getActual() + 1,new Municipio());
		 	}else{
		 		vistaMunicipio vista=new vistaMunicipio(this);
		 		vista=(vistaMunicipio)this.getSession().getVista();
	  ...
	  public void primero(){
		  if (this.getSession().getListaDto().size()>0){
		    	this.getSession().setDtoActual(0,new Municipio());
		 	}else{
		 		vistaMunicipio vista=new vistaMunicipio(this);
		 		vista=(vistaMunicipio)this.getSession().getVista();
		 		vista.mensageDialogo("error","No Existen Datos que mostrar." , vista.getTitle());
	  ...
	  public void ultimo(){
		  if (this.getSession().getListaDto().size()>0){
		       this.getSession().setDtoActual(this.getSession().getListaDto().size() - 1,new Municipio());
			}else{
				vistaMunicipio vista=new vistaMunicipio(this);
				vista=(vistaMunicipio)this.getSession().getVista();
				vista.mensageDialogo("error","No Existen Datos que mostrar." , vista.getTitle());
  ...
  public void mouseClicked(MouseEvent e) {
  	if (e.getClickCount() == 2) {
          ...
          this.getSession().setDtoActual(row,new Municipio());
          this.cambiarVista();       
         }	
  }
  @Override
