...
import UnidadDeMedida.vistaUnidadDeMedida;
...
public class controladorUnidadDeMedida extends ControladorBase implements ActionListener, MouseListener,KeyListener {
		
	public controladorUnidadDeMedida(){
		vistaUnidadDeMedida vista=new vistaUnidadDeMedida(this);
		UnidadDeMedida dto=new UnidadDeMedida();
	...
	public void actualizarVista(){
		vistaUnidadDeMedida vista=(vistaUnidadDeMedida)this.getSession().getVista();
		UnidadDeMedida dto=new UnidadDeMedida();
		modeloUnidadDeMedida modUnidadDeMedida=new modeloUnidadDeMedida();
		this.getSession().setListaDto(modUnidadDeMedida.buscar(dto));
		modeloReporte modeloReporte=new modeloReporte();
		javax.swing.DefaultComboBoxModel modeloComboReporte=new javax.swing.DefaultComboBoxModel(modeloReporte.buscar(new Reporte(),"strpadre","UnidadDeMedida").toArray());
		...
		modeloComboMaestro=new javax.swing.DefaultComboBoxModel(modMaestro.hijosPorMaestro("TIPO_UNIDAD_MEDIDA"));
		vista.getCmbdtotipo_unidad().setModelo(modeloComboMaestro);
		this.getSession().setDtoActual(0,dto);
		this.cargarGrid();
	}
	public void nuevo(){
		vistaUnidadDeMedida vista=new vistaUnidadDeMedida(this);
		vista=(vistaUnidadDeMedida)this.getSession().getVista();
		vista.setDto(new UnidadDeMedida());
		vista.chkbolpredeterminado.setSelected(false);
	...
	public void grabar(){
	    ...
	    if (strlngid.isEmpty() || strlngid.length()==0){
	        ...
	        if (lsError.isEmpty()) {
	  	      ...
	  	      if (JOptionPane.YES_OPTION == intDialogo){
	               modeloUnidadDeMedida modelo=new modeloUnidadDeMedida();
	               modelo.grabar(dto);
	  ...
	  public void buscar(){
	    ...
	    if (str.isEmpty() || str.length()==0 || str.contentEquals("0")){
	       vista.mensageDialogo("error","Valor de búsqueda vacío o inválido.\nIntroduzca un número mayor que cero." , vista.getTitle());
	    	...
	    	modeloUnidadDeMedida modelo=new modeloUnidadDeMedida();
	        UnidadDeMedida dto=new UnidadDeMedida();
	        dto=modelo.buscarUnidadDeMedida(new UnidadDeMedida(),"lngid",str);
	        vista.setDto(dto);		
	    }
	  }
	  ...
	  public void siguiente(){
	    if (this.getSession().getListaDto().size()>0){
	    	this.getSession().setDtoActual(this.getSession().getActual() + 1,new UnidadDeMedida());
	 	}else{
	 		vistaUnidadDeMedida vista=new vistaUnidadDeMedida(this);
	 		vista=(vistaUnidadDeMedida)this.getSession().getVista();
	  ...
	  public void primero(){
	    if (this.getSession().getListaDto().size()>0){
	    	this.getSession().setDtoActual(0,new UnidadDeMedida());
	 	}else{
	 		vistaUnidadDeMedida vista=new vistaUnidadDeMedida(this);
	 		vista=(vistaUnidadDeMedida)this.getSession().getVista();
	 		vista.mensageDialogo("error","No Existen Datos que mostrar." , vista.getTitle());
	  ...
	  public void ultimo(){
		if (this.getSession().getListaDto().size()>0){
	       this.getSession().setDtoActual(this.getSession().getListaDto().size() - 1,new UnidadDeMedida());
		}else{
			vistaUnidadDeMedida vista=new vistaUnidadDeMedida(this);
			vista=(vistaUnidadDeMedida)this.getSession().getVista();
			vista.mensageDialogo("error","No Existen Datos que mostrar." , vista.getTitle());
	...
	public void mouseClicked(  MouseEvent e){
		if (e.getClickCount() == 2) {
			...
			this.getSession().setDtoActual(row,new UnidadDeMedida());
			this.cambiarVista();
		}
	}
