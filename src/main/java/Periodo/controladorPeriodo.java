...
import Periodo.vistaPeriodo;
import Organizacion.modeloOrganizacion;
public class controladorPeriodo extends ControladorBase implements ActionListener, MouseListener,KeyListener {
	
	public controladorPeriodo(){
		vistaPeriodo vista=new vistaPeriodo(this);
	...
	public void actualizarVista(){
		vistaPeriodo vista=(vistaPeriodo)this.getSession().getVista();
		Periodo dto=new Periodo();
		modeloPeriodo modPeriodo=new modeloPeriodo();
		this.getSession().setListaDto(modPeriodo.buscar(dto));
		modeloReporte modeloReporte=new modeloReporte();
		javax.swing.DefaultComboBoxModel modeloComboReporte=new javax.swing.DefaultComboBoxModel(modeloReporte.buscar(new Reporte(),"strpadre","Periodo").toArray());
	...
	public void nuevo(){
	    vistaPeriodo vista=new vistaPeriodo(this);
	    vista=(vistaPeriodo)this.getSession().getVista();
	    vista.setDto(new Periodo());
	    ...
	    if (strlngid.isEmpty() || strlngid.length()==0){
	        ...
	        if (lsError.isEmpty()) {
	  	      ...
	  	      if (JOptionPane.YES_OPTION == intDialogo){
	               modeloPeriodo modelo=new modeloPeriodo();
	               modelo.grabar(dto);
	  ...
	  public void buscar(){
	    ...
	    if (str.isEmpty() || str.length()==0 || str.contentEquals("0")){
	       vista.mensageDialogo("error","Valor de búsqueda vacío o inválido.\nIntroduzca un número mayor que cero." , vista.getTitle());
	    	...
	    	modeloPeriodo modelo=new modeloPeriodo();
	        Periodo dto=new Periodo();
	        dto=modelo.buscarPeriodo(new Periodo(),"lngid",str);
	        vista.setDto(dto);		
	    }
	  }
	  ...
	  public void siguiente(){
	    if (this.getSession().getListaDto().size()>0){
	    	this.getSession().setDtoActual(this.getSession().getActual() + 1,new Periodo());
	 	}else{
	 		vistaPeriodo vista=new vistaPeriodo(this);
	 		vista=(vistaPeriodo)this.getSession().getVista();
	  ...
	  public void primero(){
	    if (this.getSession().getListaDto().size()>0){
	    	this.getSession().setDtoActual(0,new Periodo());
	 	}else{
	 		vistaPeriodo vista=new vistaPeriodo(this);
	 		vista=(vistaPeriodo)this.getSession().getVista();
	 		vista.mensageDialogo("error","No Existen Datos que mostrar." , vista.getTitle());
	  ...
	  public void ultimo(){
		if (this.getSession().getListaDto().size()>0){
	       this.getSession().setDtoActual(this.getSession().getListaDto().size() - 1,new Periodo());
		}else{
			vistaPeriodo vista=new vistaPeriodo(this);
			vista=(vistaPeriodo)this.getSession().getVista();
			vista.mensageDialogo("error","No Existen Datos que mostrar." , vista.getTitle());
	...
	public void mouseClicked(  MouseEvent e){
		if (e.getClickCount() == 2) {
			...
			this.getSession().setDtoActual(row,new Periodo());
			this.cambiarVista();
		}
	}
