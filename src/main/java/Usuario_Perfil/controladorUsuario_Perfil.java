...
import Usuario_Perfil.modeloUsuario_Perfil;
import Usuario_Perfil.vistaUsuario_Perfil;
import Organizacion.modeloOrganizacion;
import Perfil.modeloPerfil;
import Usuario.modeloUsuario;
public class controladorUsuario_Perfil extends ControladorBase implements ActionListener, MouseListener,KeyListener {
  public controladorUsuario_Perfil(){
    vistaUsuario_Perfil vista=new vistaUsuario_Perfil(this);
    Usuario_Perfil dto=new Usuario_Perfil();
  ...
  public void actualizarVista(){
    vistaUsuario_Perfil vista=(vistaUsuario_Perfil)this.getSession().getVista();
    Usuario_Perfil dto=new Usuario_Perfil();
    modeloUsuario_Perfil modUsuario_Perfil=new modeloUsuario_Perfil();
    this.getSession().setListaDto(modUsuario_Perfil.buscar(dto));
    modeloReporte modeloReporte=new modeloReporte();
    javax.swing.DefaultComboBoxModel modeloComboReporte=new javax.swing.DefaultComboBoxModel(modeloReporte.buscar(new Reporte(),"strpadre","Usuario_Perfil").toArray());
    ...
    vista.getLstdto_usuario().setModelDes(modeloUsuario.buscarUsuarios());
    this.getSession().setDtoActual(0,dto);
    this.cargarGrid();
  }
  public void nuevo(){
	    vistaUsuario_Perfil vista=new vistaUsuario_Perfil(this);
	    vista=(vistaUsuario_Perfil)this.getSession().getVista();
	    vista.setDto(new Usuario_Perfil());
	    ...
	    if (strlngid.isEmpty() || strlngid.length()==0){
	        ...
	        if (lsError.isEmpty()) {
	  	      ...
	  	      if (JOptionPane.YES_OPTION == intDialogo){
	               modeloUsuario_Perfil modelo=new modeloUsuario_Perfil();
	               modelo.grabar(dto);
	  ...
	  public void buscar(){
	    ...
	    if (str.isEmpty() || str.length()==0 || str.contentEquals("0")){
	       vista.mensageDialogo("error","Valor de búsqueda vacío o inválido.\nIntroduzca un número mayor que cero." , vista.getTitle());
	    	...
	    	modeloUsuario_Perfil modelo=new modeloUsuario_Perfil();
	        Usuario_Perfil dto=new Usuario_Perfil();
	        dto=modelo.buscarUsuario_Perfil(new Usuario_Perfil(),"lngid",str);
	        vista.setDto(dto);		
	    }
	  }
	  ...
	  public void siguiente(){
	    if (this.getSession().getListaDto().size()>0){
	    	this.getSession().setDtoActual(this.getSession().getActual() + 1,new Usuario_Perfil());
	 	}else{
	 		vistaUsuario_Perfil vista=new vistaUsuario_Perfil(this);
	 		vista=(vistaUsuario_Perfil)this.getSession().getVista();
	  ...
	  public void primero(){
	    if (this.getSession().getListaDto().size()>0){
	    	this.getSession().setDtoActual(0,new Usuario_Perfil());
	 	}else{
	 		vistaUsuario_Perfil vista=new vistaUsuario_Perfil(this);
	 		vista=(vistaUsuario_Perfil)this.getSession().getVista();
	 		vista.mensageDialogo("error","No Existen Datos que mostrar." , vista.getTitle());
	  ...
	  public void ultimo(){
		if (this.getSession().getListaDto().size()>0){
	       this.getSession().setDtoActual(this.getSession().getListaDto().size() - 1,new Usuario_Perfil());
		}else{
			vistaUsuario_Perfil vista=new vistaUsuario_Perfil(this);
			vista=(vistaUsuario_Perfil)this.getSession().getVista();
			vista.mensageDialogo("error","No Existen Datos que mostrar." , vista.getTitle());
  ...
  public void mouseClicked(  MouseEvent e){
    if (e.getClickCount() == 2) {
      ...
      this.getSession().setDtoActual(row,new Usuario_Perfil());
      this.cambiarVista();
    }
  }
